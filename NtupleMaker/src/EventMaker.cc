...
EventMaker::EventMaker(const edm::ParameterSet& iConfig) {
    ...
    produces< vector< TString > >          (branchprefix+"QJettag"        ).setBranchAlias(aliasprefix_+"_QJettag"       );
    ...
    QJettag_     = iConfig.getParameter<std::string>("QJettag");
...
void EventMaker::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    auto_ptr<vector<TString>>             evt_QJettag         (new vector<TString>           );
    ...
    *evt_isRealData                = iEvent.isRealData();
    ...
    evt_QJettag->push_back( TString( QJettag_    .c_str() ) );
...
//	   cout << "dcsHandle is valid. The current: " << current << endl;
    ...
    iEvent.put(evt_QJettag          ,branchprefix+"QJettag"         );
